...
func newRootCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Args: func(cmd *cobra.Command, args []string) error {
			...
			if opts.kubeconfig != "" && strings.TrimSpace(opts.kubeconfig) == "" {
				return errors.New("kubeconfig path cannot be empty")
			}
			if opts.kubeconfig != "" {
				opts.kubeconfig = filepath.Clean(opts.kubeconfig)
			}
			if opts.contextName != "" && strings.TrimSpace(opts.contextName) == "" {
...
func hasOperationalFlags(cmd *cobra.Command) bool {
	hasFlags := false
