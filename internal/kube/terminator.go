...
var ErrNoTerminatingNamespaces = errors.New("no namespaces are currently stuck in Terminating")
...
func BuildRESTConfig(cfg Config) (*rest.Config, error) {
	if cfg.Kubeconfig != "" {
		if _, err := os.Stat(cfg.Kubeconfig); err != nil {
			if errors.Is(err, os.ErrNotExist) {
				return nil, fmt.Errorf("kubeconfig file not found: %s", cfg.Kubeconfig)
			}
			return nil, fmt.Errorf("read kubeconfig file %q: %w", cfg.Kubeconfig, err)
		}
	}
	...
	if selectedContext == "" {
		...
		if selectedContext == "" {
			return nil, errors.New("kubernetes context is not set; use --context or configure current-context")
		}
	}
	if _, ok := rawConfig.Contexts[selectedContext]; !ok {
		if cfg.Context != "" {
			return nil, fmt.Errorf("kubernetes context %q was not found in kubeconfig", cfg.Context)
		}
		return nil, fmt.Errorf("current kubernetes context %q was not found in kubeconfig", selectedContext)
	}
...
func ResolveTargets(ctx context.Context, client kubernetes.Interface, names []string, allTerminating bool, labelSelector string) ([]string, error) {
	...
	case allTerminating:
		...
		if len(targets) == 0 {
			return nil, ErrNoTerminatingNamespaces
		}
		return targets, nil
	case len(names) > 0:
